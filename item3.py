...
@app.route('/') 
def index():
    return 'REALIZANDO ITEM 3 EXAMEN FINAL DE PROGRAMACION DE REDES VIRTUALIZADAS!!!'
...
def signup_v2():
    conn = sqlite3.connect(db_name) 
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS USER_HASH (USERNAME TEXT	PRIMARY KEY NOT NULL,
HASH	TEXT	NOT NULL);''')
    conn.commit() 
    try:
